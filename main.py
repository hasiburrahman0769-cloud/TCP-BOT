...
📱 Player will see:
"MG24_GAMER wants to join your team"
...
async def generate_guest_accounts(count=1, name="MG24", password_prefix="FF"):
...
async def generate_and_save_accounts(count, name="MG24", password_prefix="FF"):
...
async def MG24_GAMER(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]ㅤHꫝsɪʙㅤ6T9 [00FF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[00E5FF]TELEGRAM : @hasib_hi\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
Region: {server_name}
[FFDD00]°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
[00FF00][b][c]BOT MADE BY MG24_GAMER 
"""
            else:
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@MG24_GAMER_PRO
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="MG24_GAMER", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="MG24_GAMER", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde MG24_GAMER ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="MG24_GAMER", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        if "2799233875" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 14661237406
                                message_text = f" Can't accept Your request Talk to ㅤHꫝsɪʙㅤ6T9"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF00FF]█▓▒░░ WELCOME TO ㅤHꫝsɪʙㅤ6T9 BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000]ㅤHꫝsɪʙㅤ6T9
[FFFFFF]🎯 STATUS: [00FF00]ㅤHꫝsɪʙㅤ6T9
[FFFFFF]📱 CONTACT: [FFFF00]@hasib_hi
[00FF00]»»————-　★　————-««
[00E5FF]█▓▒░░ ENJOY THE BOT! ░░▒▓█"""
                ...
                if senthi == True:
                    ...
                    def get_random_color(): return "_" 
                    message = """[B][C][FF0000]█▓▒░░ WELCOME TO ㅤHꫝsɪʙㅤ6T9 BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000]ㅤHꫝsɪʙㅤ6T9X³
[FFFFFF]🎯 STATUS: [00FF00 Hꫝsɪʙㅤ6T9 TCP
[FFFFFF]📱 CONTACT: [FFFF00]@hasib_hi
[00FF00]»»————-　★　————-««
[00E5FF]█▓▒░░ ENJOY THE BOT! ░░▒▓█"""
                        # In your auto-join (Old Handler) code, find this line:
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by MG24_GAMER
/bio Level 70 | Pro Player
/bio Add me: MG24_GAMER
...
[b][i][FFC0CB]youtube: MG24_GAMER 99[/b]
...
[C][B][0000FF] Created by MG24_GAMER
Modified by - MG24_GAMER
"""
...
#=============================BAN CHECK=================================
                        ...
                        BLOCKED_NAMES = ["arbaz", "ARBAZ", "Arbaz", "MG24_GAMER"]  # Add your actual name
...
"""
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            basic = """
[c][b]01.[00FF00]/start-[FFFFFF]Start match
02.[00FF00]/exit-[FFFFFF]Leave squad
03.[00FF00]/3-[FFFFFF]Send 3-player invite
04.[00FF00]/5-[FFFFFF]Send 5-player invite
05.[00FF00]/6-[FFFFFF]Send 6-player invite
06.[00FF00]! (team code)-[FFFFFF]Join squad
07.[00FF00]/ghost (team code)-[FFFFFF]Ghost join squad
08.[00FF00]/e [emote]-[FFFFFF]Send emote to yourself
09.[00FF00]/e [uid] [emote]-[FFFFFF]Send emote to player
10.[00FF00]/e list-[FFFFFF]Show all emotes
...
"""
                            ...
                            emotes = """[c][b]12.[00FF00][00FF00]@evos-[FFFFFF]Start evolution cycle
13.[00FF00]@sevos-[FFFFFF]Stop evolution cycle
14.[00FF00]/evo [num]-[FFFFFF]Send evo emote (1-18)
15.[00FF00]/evo [uid] [num]-[FFFFFF]Send evo emote to player
16.[00FF00/fast [uid] [emote]-[FFFFFF]Fast emote spam
17.[00FF00]/p [uid] [emote] [num]-[FFFFFF]Custom emote spam
18.[00FF00]/reject [uid]-[FFFFFF]Reject spam
19.[00FF00]/reject_stop-[FFFFFF]Stop reject spam
"""
...
21.[00FF00]/bundle-[FFFFFF]Get Bundle list
22.[00FF00]/bundle [name]-[FFFFFF]Set Bundle 
23.[00FF00]/msg [text] [times]-[FFFFFF]Message spam
24.[00FF00]/stop msg-[FFFFFF]Stop message spam
25.[00FF00]/mg [text] [repeats]-[FFFFFF]Wave message spam
26.[00FF00]/inv [uid]-[FFFFFF]Send group invite
27.[00FF00]/joinroom [id] [pass] -[FFFFFF]Join custom room
28.[00FF00]/lag [team_code]-[FFFFFF]Lag attack squad
29.[00FF00]/stop lag-[FFFFFF]Stop lag attack"""

                            await safe_send_message(response.Data.chat_type, spam_cmnd, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            badge_cmnd = """[c][b]30.[00FF00]/inv [uid]-[FFFFFF]Send group invite
31.[00FF00]/joinroom [id] [pas] [FFFFFF]-Join custom room
32.[00FF00]/lag [team_code]-[FFFFFF]Lag attack squad
33.[00FF00]/stop lag-[FFFFFF]Stop lag attack
34.[00FF00]/info [uid]-[FFFFFF]Player information
35.[00FF00]/status [uid]-[FFFFFF]Check player status
36 [00FF00]/gali [name]-[FFFFFF]Gali spam message
37.[00FF00]/likes [uid]-[FFFFFF]Send 100 likes
...
40.[00FF00]/wladd (uid)-[FFFFFF]Add UID to whitelist
41.[00FF00]/wlremove (uid)-[FFFFFF]Remove UID from whitelist
42.[00FF00]/help-[FFFFFF]Show this help menu
43.[00FF00]/admin-[FFFFFF]Admin Info
44.[00FF00]/quick [cd] [em] [id]-[FFFFFF]Quik emote
45.[00FF00]/start-[FFFFFF]Start Chat Mode
...
async def MaiiiinE():
    ...
    if not credentials:
        ...
        print("📝 Format: uid=4487767436,password=C4BF8EB2F68B33EB9A29B6B16BF0AB45CCAC845DB85E2B2EFFA3DF7861B387C3")
        return None
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 MG24_GAMER BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 MG24_GAMER BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
